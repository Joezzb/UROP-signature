# -*- coding: utf-8 -*-
"""
Created on Sun Aug  2 15:40:03 2020
...
class preprocess:
    def __init__(self):
        ...
        self.collection_50_test=np.empty([3498,50,50])
        self.level=2
        ...
        self.collection_50_train=np.empty([7494,50,50])
    def process(self):
        #test_process
        n=0
        for i in range(3498):
    ...
    def image_50(self):
        ...
        for i in range(3498):
            ...
            for k in range(300):
                ...
                if self.data[i,k,0]==0:
                    break
                temp_map[int(self.data[i,k,0]/10),int(self.data[i,k,1]/10)]=1
            self.collection_50_test[i,:,:]=temp_map   
         #trian
        for i in range(7494):
            temp_map=np.zeros([50,50])
            for k in range(300):
                ...
                if self.data_train[i,k,0]==0:
                    break
                temp_map[int(self.data_train[i,k,0]/10),int(self.data_train[i,k,1]/10)]=1
            self.collection_50_train[i,:,:]=temp_map 
    ...
    def siglayer(self,level=2):
        ...
        #test
        self.sigcollection_test=np.zeros([3498,50,50,depth])
        for i in range(3498):
            k=2
            while self.data[i,k+2,0]!=0:
                if self.data[i,k-2:k+3,0].__contains__(0.01):
                    ...
                    self.sigcollection_test[i,int(self.data[i,k,0]/10),
                                       int(self.data[i,k,1]/10),:]+=sig.sig(
                            self.data[i,k-2:k+3,:],self.level)
                k+=1      
        #train
        self.sigcollection_train=np.zeros([7494,50,50,depth])
        for i in range(7494):
            k=2
            while self.data_train[i,k+2,0]!=0:
                if self.data_train[i,k-2:k+3,0].__contains__(0.01):
                    ...
                    self.sigcollection_train[i,int(self.data_train[i,k,0]/10),
                                       int(self.data_train[i,k,1]/10),:]+=sig.sig(
                            self.data_train[i,k-2:k+3,:],self.level)
                k+=1       
            
    def siglayer_av(self,level=2):
        self.level=level
        depth=2*(2**self.level-1) 
        #test
        self.sigcollection_test=np.zeros([3498,50,50,depth])
